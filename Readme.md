...
evolucionada con una **migración a arquitectura modular DDD** y un **pipeline de CI/CD**,
más prácticas de calidad y seguridad de software.

> El tutorial conceptual original sobre Domain-Driven Design (DDD) se conserva en
> [`docs/DDD-Tutorial.md`](docs/DDD-Tutorial.md). Este documento es el **entregable del
> proyecto final**.
...
implementada bajo una arquitectura **Domain-Driven Design (DDD)** modular.
...
- Programación de transacciones recurrentes (diaria, semanal, mensual, días hábiles).
...
Arquitectura **modular DDD**, migrada desde capas técnicas globales hacia módulos por
*bounded context* (ver [sección 13](#13-migración-a-arquitectura-modular-ddd-práctica-07)
...
modules/<nombre>/
    ...
    infrastructure/  → Implementación de repositorios (JPA)
    presentation/    → Controladores REST
```

Conceptos DDD aplicados: **Entidades, Value Objects, Agregados (un repositorio por agregado),
Servicios, DTOs y contextos delimitados**. Detalle conceptual en
[`docs/DDD-Tutorial.md`](docs/DDD-Tutorial.md) y [`UML.md`](UML.md).
...
| Ledger (Account/Category/Transaction) | Cuentas, categorías y movimientos | `POST /persons/{email}/accounts`, `POST /persons/{email}/categories`, `POST/PUT/DELETE /persons/{email}/ledgers/records` |
| Group | Grupos y administración | `POST /groups`, `GET /persons/{email}/groups`, `GET /persons/{email}/groups/{denom}/accounts` |
| Scheduling | Transacciones programadas recurrentes | Programación con periodicidad diaria/semanal/mensual/días hábiles |
...
- **Backend:** Java 8 (compatible JDK 21 con Lombok ≥1.18.30), Spring Boot 2.2, Spring Web, Spring HATEOAS, Spring Security.
- **ORM / BD:** Spring Data JPA + Hibernate sobre **H2** (en memoria) / PostgreSQL (Docker).
- **Build:** Maven (con Maven Wrapper). **Calidad:** SonarQube + JaCoCo. **Tests:** JUnit 5 + Mockito.
...
- **Unitarias e integración:** 1419 pruebas con **JUnit 5 + Mockito** (100% pasando tras las
  correcciones de seguridad).
  ```bash
  cd backend && mvn test
  ```
- **Funcionales:** Newman/Postman (`backend/src/test/resources/finance-api-tests.json`).
- **Performance:** JMeter (`backend/src/test/jmeter/performance-test.jmx`).
- **Seguridad:** OWASP ZAP (`security/zap-scan.sh`).
...
|----|----------------|-------|--------|
| V1 | Sin autenticación/contraseña (IDOR — acceso a datos de cualquier persona) | A01 | Corregido |
| V2 | CORS abierto `@CrossOrigin(origins="*")` en transacciones | A05 | Corregido |
| V3 | Consola H2 expuesta sin protección | A05 | Corregido |
| V4 | Sin validación de entrada (`@Valid`) → HTTP 500 | A03 | Corregido |
| V5 | Dependencias vulnerables (npm/Maven) | A06 | Mitigado parcialmente |
| V6 | Sin CSRF ni cabeceras de seguridad | A05 | Corregido |
| V7 | Fuga de información en errores (stacktrace) | A04 | Corregido |
...
Build → Static Analysis (SonarQube) → Unit Tests (JUnit/Mockito)
      → Functional Tests (Newman) → Performance (JMeter)
...
- **GitHub Projects** (Kanban): TO-DO → CURRENT ITERATION → IN PROGRESS → FIX VALIDATION → DONE.
- Etiquetas: `Nuevo Requisito`, `Mejora` (refactor/smell), `Corrección` (bug/vulnerabilidad).
- Flujo de release: GitHub Project → Issues → Commits → Pull Request → Merge.
